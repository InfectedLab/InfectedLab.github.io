...
1.  **Proje Günlükleri:** Üzerinde çalıştığım uygulamalar.
2.  **Kod Parçacıkları:** Hayat kurtaran scriptler.
3.  **Linux & Siber Güvenlik:** Terminal maceralarım.

### 💻 Küçük Bir Kod Örneği

Blogun kod renklendirme (syntax highlighting) özelliğini de test etmiş olalım. İşte basit bir Python fonksiyonu:

```python
def welcome_to_lab(name):
    print(f"Hoş geldin {name}!")
    print("InfectedLab sistemleri aktif...")

welcome_to_lab("Ziyaretçi")